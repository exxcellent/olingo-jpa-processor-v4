...
class IntermediateEntityContainer extends IntermediateModelElement {
	...
	IntermediateEntityContainer(final JPAEdmNameBuilder nameBuilder, final ServiceDocument serviceDocument)
		...
		for (final AbstractJPASchema schema : serviceDocument.getJPASchemas()) {
			...
			if (functions != null) {
				for (final JPAFunction jpaFu : functions) {
					if (((IntermediateFunction) jpaFu).isBound() == false
							&& ((IntermediateFunction) jpaFu).hasFunctionImport()) {
