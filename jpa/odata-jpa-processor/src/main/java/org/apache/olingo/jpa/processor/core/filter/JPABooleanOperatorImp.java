...
class JPABooleanOperatorImp implements JPABooleanOperator {
	...
	private final JPAExpression<Expression<Boolean>> left;
	private final JPAExpression<Expression<Boolean>> right;

	public JPABooleanOperatorImp(final JPAODataDatabaseProcessor converter, final BinaryOperatorKind operator,
			final JPAExpression<Expression<Boolean>> left, final JPAExpression<Expression<Boolean>> right) {
		super();
