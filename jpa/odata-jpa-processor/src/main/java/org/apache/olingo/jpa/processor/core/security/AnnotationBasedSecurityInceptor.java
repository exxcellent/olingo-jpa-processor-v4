...
public class AnnotationBasedSecurityInceptor implements SecurityInceptor {
  ...
  /**
   * The {@link SecurityConfiguration} is the runtime way to set the same values as via
   * {@link ODataOperationAccess @ODataOperationAccess} annotation.
   *
   * @param globalDefaultEntitySecurityConfiguration Define a default behaviour for the security check on entity access.
   * Without this (same as <code>null</code>) no default security for unconfigured entitites is enabled,
   * @param globalDefaultOperationSecurityConfiguration Define a default behaviour for the security check on action
   * access. Without this (same as <code>null</code>) no default security for actions is defined and the entity level
