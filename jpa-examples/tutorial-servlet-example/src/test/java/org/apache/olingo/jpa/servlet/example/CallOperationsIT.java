...
//		URIBuilder uriBuilder = endpoint.newUri().appendOperationCallSegment("ClearPersonsCustomStrings");
		URIBuilder uriBuilder = endpoint.newUri().appendEntitySetSegment("Persons").appendKeySegment("99").appendOperationCallSegment(Constant.PUNIT_NAME+".ClearPersonsCustomStrings");
		Map<String, ClientValue> functionParameters = new HashMap<>();
		ODataInvokeResponse<ClientInvokeResult> response = endpoint.callAction(uriBuilder, ClientInvokeResult.class,functionParameters);
	...
	public void testActionBound2() throws Exception {
		URIBuilder uriBuilder = endpoint.newUri().appendEntitySetSegment("Persons").appendKeySegment("99").appendOperationCallSegment(Constant.PUNIT_NAME+".DoNothingAction1");
		Map<String, ClientValue> functionParameters = new HashMap<>();
		functionParameters.put("affectedPersons", new ClientCollectionValueImpl<ClientPrimitiveValue>(EdmPrimitiveTypeKind.String.getFullQualifiedName().getName()));
		...
		ODataInvokeResponse<ClientEntity> response = endpoint.callAction(uriBuilder, ClientEntity.class,functionParameters);
	...
	public void testActionBound3() throws Exception {
		URIBuilder uriBuilder = endpoint.newUri().appendEntitySetSegment("Persons").appendKeySegment("99").appendOperationCallSegment(Constant.PUNIT_NAME+".SendBackTheInput");
		Map<String, ClientValue> functionParameters = new HashMap<>();
		String teststring = "teststring";
		functionParameters.put("input", new ClientPrimitiveValueImpl.BuilderImpl().buildString(teststring));
		ODataInvokeResponse<ClientProperty> response = endpoint.callAction(uriBuilder, ClientProperty.class,functionParameters);
		Assert.assertTrue(response.getStatusCode() == HttpStatusCode.OK.getStatusCode());
		String retValue = response.getBody().getPrimitiveValue().toCastValue(String.class);
	...
	public void testFunctionUnbound() throws Exception {
		URIBuilder uriBuilder = endpoint.newUri().appendEntitySetSegment("Persons").appendOperationCallSegment("IS_PRIME");
		Map<String, ClientValue> functionParameters = new HashMap<>();
		functionParameters.put("Number", new ClientPrimitiveValueImpl.BuilderImpl().buildDecimal(BigDecimal.valueOf(123.456)));
		ODataInvokeResponse<ClientInvokeResult> response = endpoint.callFunction(uriBuilder, ClientInvokeResult.class,functionParameters);
		Assert.assertTrue(response.getStatusCode() == HttpStatusCode.OK.getStatusCode());
